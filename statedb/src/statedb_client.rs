...
use statedb_service::{
    Fea, FlushResponse, GetProgramRequest, GetProgramResponse, GetRequest,
    GetResponse, SetProgramRequest,
    SetProgramResponse, SetRequest, SetResponse,
};
...
impl StateDBClientCli {
    pub async fn new(addr: String) -> Result<Self, EigenError> {
        let result = StateDbServiceClient::connect(addr.clone()).await.map_err(|e| EigenError::from(format!("connect err: {:?}", e)));
        match result {
            Ok(c) => Ok(
                StateDBClientCli{
                    client: c
    ...
    pub async fn set(mut self, root: Fea, key: Fea, val: String) -> Result<Response<SetResponse>, EigenError> {
    ...
    pub async fn get_program(mut self, key: Fea) -> Result<Response<GetProgramResponse>, EigenError> {
    ...
    pub async fn set_program(mut self, key: Fea, data: Vec<u8>) -> Result<Response<SetProgramResponse>, EigenError> {
