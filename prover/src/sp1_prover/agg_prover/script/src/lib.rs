...
impl Prover<AggContext> for AggProver {
    fn prove(&self, ctx: &AggContext) -> Result<()> {
        ...
        agg_proof.save(ctx.task_path.clone()).expect("saving proof failed");
...
mod tests {
    ...
    fn test_agg_prove() {
        ...
        agg_context.input = "/mnt/nfs/sy/eigen-prover/prover/src/sp1_prover/script/sp1_proof_68855.bin".to_string();
        agg_context.input2 = "/mnt/nfs/sy/eigen-prover/prover/src/sp1_prover/script/sp1_proof_68858.bin".to_string();

        agg_context.task_path = "../../test_vectors/proof/0".to_string();
    ...
    fn test_ark_groth16() {
        ...
        let GROTH16_VK_BYTES: &'static [u8] = include_bytes!("../../../test_vectors/groth16_vk.bin");
