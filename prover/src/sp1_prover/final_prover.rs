...
impl Prover<FinalContext> for Sp1FinalProver {
    fn prove(&self, ctx: &FinalContext) -> Result<()> {
        ...
        let vk: VerifyingKey<ark_ec::bls12::Bls12<ark_bls12_381::Config>> = VerifyingKey::<Bls12_381> {
            alpha_g1: vk.alpha_1,
            beta_g2: vk.beta_2,
            gamma_g2: vk.gamma_2,
            delta_g2: vk.delta_2,
        ...
        let proof: Proof<ark_ec::bls12::Bls12<ark_bls12_381::Config>> = Proof { a: proof.pi_a, b: proof.pi_b, c: proof.pi_c };
...
mod tests {
    ...
    use prover_core::prover::Prover;
    // 定义泛型函数替代闭包
    fn serialize_to_bytes<T: CanonicalSerialize>(obj: &T) -> Vec<u8> {
        let mut bytes = Vec::new();
        obj.serialize_compressed(&mut bytes).expect("Serialization failed");
        bytes
    }
    ...
    fn test_sp1_proof() {
        env_logger::try_init().unwrap_or_default();
        let sp1_prover = Sp1FinalProver::default();
        let output_dir = "/mnt/data/sy/eigen-prover/prover/src/sp1_prover/test_data";
        let input_file_bls12381 = Path::new(&output_dir).join("public_inputs_bls12381.json");
        println!("input_file_bls12381: {:?}", input_file_bls12381);
        let vk_file_bls12381 = Path::new(&output_dir).join("groth16_vk_bls12381.json");
        let proof_file_bls12381 = Path::new(&output_dir).join("proof_bls12381.json");
        ...
        // 序列化验证密钥和证明
        let vk_bytes = serialize_to_bytes(&vk);
        let proof_bytes = serialize_to_bytes(&proof);

        // 单独序列化公共输入（切片）
        let mut public_inputs_bytes = Vec::new();
        public_inputs.serialize_compressed(&mut public_inputs_bytes).expect("Failed to serialize public inputs");
        ...
        let test_data_dir = Path::new(&output_dir);
        fs::write(test_data_dir.join("vk.bin"), &vk_bytes).expect("Failed to write vk.bin");
        fs::write(test_data_dir.join("proof.bin"), &proof_bytes).expect("Failed to write proof.bin");
        fs::write(test_data_dir.join("public_inputs.bin"), &public_inputs_bytes).expect("Failed to write public_inputs.bin");

        // 验证过程
        let vk_prepared = ark_groth16::prepare_verifying_key(&vk);
        let res = Groth16::<Bls12_381>::verify_proof(&vk_prepared, &proof, public_inputs);
        assert!(res.is_ok(), "Groth16 proof verification failed: {:?}", res.err());
    }
