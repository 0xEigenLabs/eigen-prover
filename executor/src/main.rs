...
impl Executor {
    ...
    fn exec(&self, block_json: &str, chain_id: u64){
        ...
        let parsed_json: Result<Value> = serde_json::from_str(&block_json);
        match parsed_json {
            Ok(json_value) => {
                if let (Some(currentBaseFee), Some(currentCoinbase)) = (
                    json_value.get("add").and_then(|add| add.get("env").and_then(|info| info.get("currentBaseFee"))),
                    json_value.get("add").and_then(|add| add.get("env").and_then(|info| info.get("currentCoinbase"))),
                ) {
                    println!("Value of 'currentBaseFee' field: {}", comment);
                    println!("Value of 'currentCoinbase' field: {}", hash);
                } else {
                    eprintln!("One or more fields not found in JSON");
                }
            }
            Err(err) => {
